...
def create_windows_exe(love_file, love_version, arch, output_dir):
    ...
    game_exe = love_dir / "snowflakes.exe"
    
    print(f"Creating snowflakes.exe ({arch}-bit)...")
    ...
    # Create zip archive
    output_zip = output_dir / f"snowflakes-win{arch}.zip"
...
def main():
    ...
    web = releases / "web"
    love_file = releases / "snowflakes.love"
    ...
    if is_wsl:
        ...
        lovejs_cmd = f"npx love.js.cmd -c \"{love_file_win}\" \"{web_win}\" --title \"snowflakes\""
    else:
        lovejs_cmd = f"npx love.js.cmd -c \"{love_file}\" \"{web}\" --title \"snowflakes\""
    ...
    checks = {
        ".love file": love_file,
        "Windows 32-bit": executables / "snowflakes-win32.zip",
        "Windows 64-bit": executables / "snowflakes-win64.zip", 
